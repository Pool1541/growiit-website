...
export default function Contact() {
  ...
  const handleSubmit: handleSubmit<typeof validationSchema> = async function (values,{ setSubmitting }) {
    ...
    try {
      ...
      if (!response.ok) throw Error('Error en el envío del formulario')
    ...
    <section id='contact' className={`${styles.contact} landing-section`} data-header-color='black'>
      <Wrapper className={`${styles['flex-wrapper']}`}>
        <div className={styles.left}>
          <div className={clsx(styles['success-message'], { [styles.hidden]: status !== "success" })}>
            <MailBoxIcon />
            <Title as='h2'>¡Gracias por contactarnos!</Title>
            <p>
          ...
          <div className={clsx({ [styles.hidden]: status !== "idle" && status !== "error" })}>
            <div className={styles.header}>
              <Title as='h2'>Hablemos de tu próximo proyecto</Title>
              <p>
                Queremos conocerte y entender tus objetivos. En Growiit, trabajamos contigo para
                diseñar soluciones digitales que impulsen tu negocio al siguiente nivel. ¡Contáctanos
